import React, { useState, useCallback, useRef, DragEvent, TouchEvent, useEffect } from 'react';
            ...
            <nav className="max-w-7xl mx-auto flex justify-between items-center">
                 <div className="flex justify-center items-center gap-2 sm:gap-4 flex-grow">
                    ...
                    >
                        ...
                        <span className="font-bold text-base sm:text-lg hidden sm:inline">Instagram</span>
                    </a>
                    <a 
...
const App: React.FC = () => {
    ...
    const [promptHistory, setPromptHistory] = useState<{ data: PromptData; order: PromptPartKey[] }[]>([]);
    const [showHistory, setShowHistory] = useState(false);

    const [viewMode, setViewMode] = useState<'structured' | 'complete'>('structured');
    const [completePromptText, setCompletePromptText] = useState('');
    ...
    const [activeSidePanelKey, setActiveSidePanelKey] = useState<PromptPartKey | null>(null);
    
    // --- Chatbot State ---
    const [chatbotPersona, setChatbotPersona] = useState<Persona>('expert');
    const [chatbotConversations, setChatbotConversations] = useState<Record<Persona, ChatMessage[]>>({
        expert: [{ role: 'assistant', content: "Bonjour ! Je suis votre assistant expert en prompts. Comment puis-je vous aider à créer le prompt parfait aujourd'hui ?" }],
        generalist: [{ role: 'assistant', content: "Bonjour ! Comment puis-je vous aider aujourd'hui ?" }]
    });

    const isAppBusy = isLoading || isModifying || isAddingFieldLoading || showChatbot || showHistory || !!activeSidePanelKey;
    
    useEffect(() => {
        if (promptData) {
            setCompletePromptText(promptOrder.map(key => promptData[key].valeur).join(', '));
        }
    }, [promptData, promptOrder]);

    useEffect(() => {
        // Update expert initial message if a prompt is loaded
        setChatbotConversations(prev => {
            const expertInitialMessage = promptData
            ...
            if (prev.expert.length === 1 && prev.expert[0].role === 'assistant') {
                 return {
                    ...prev,
                    expert: [{ role: 'assistant', content: expertInitialMessage }]
                };
            }
            return prev;
        });
    ...
    const saveToHistory = useCallback(() => {
        if (promptData) {
            setPromptHistory(prev => [...prev, { data: promptData, order: promptOrder }]);
        }
    }, [promptData, promptOrder]);

    const handleRestoreFromHistory = (itemToRestore: { data: PromptData; order: PromptPartKey[] }) => {
        saveToHistory();
        setPromptData(itemToRestore.data);
        setPromptOrder(itemToRestore.order);
        ...
        const handleKeyDown = (event: KeyboardEvent) => {
            if (event.key === 'Escape') {
                if (activeSidePanelKey) {
                    ...
                    setIsAddingField(false);
                } else if (showSettings && apiConfig) { 
                    setShowSettings(false);
                }
            }
        };
    ...
    }, [isAddingField, showSettings, apiConfig, showChatbot, showHistory, activeSidePanelKey]);
    ...
    const handleCopy = async (textToCopy?: string) => {
        ...
        const text = textToCopy ?? promptOrder.map(key => promptData[key].valeur).join(', ');

        setIsTranslating(true);
        const translatedPrompt = await translateToEnglish(apiConfig, text);
    ...
    const calculateDropIndicator = (y: number) => {
        if (!itemsContainerRef.current) return;
        const items = Array.from(itemsContainerRef.current.children)
            .filter(child => child.hasAttribute('data-draggable-index'));

        let newDropIndicator = items.length;
    ...
    const handleDragOver = (e: DragEvent<HTMLDivElement>) => {
        e.preventDefault();
        calculateDropIndicator(e.clientY);
    };
    ...
    const handleTouchMove = (e: TouchEvent<HTMLDivElement>) => {
        ...
        calculateDropIndicator(e.touches[0].clientY);
    };
    ...
    // --- Side Panel Logic ---
    const handleOpenSidePanel = (key: PromptPartKey) => {
        if (!isAppBusy) {
            setActiveSidePanelKey(key);
        }
    };

    const handleUpdateFromSidePanel = (partKey: PromptPartKey, newValue: string) => {
        saveToHistory();
    ...
    const handleRefreshAlternatives = async (partKey: PromptPartKey) => {
        if (!promptData || !apiConfig) return;
        saveToHistory();
        setIsModifying(true);
        const newAlts = await getAlternatives(apiConfig, getLabel(partKey), promptData[partKey].valeur);
        if (newAlts) {
            setPromptData(prev => {
                if (!prev) return null;
                return { ...prev, [partKey]: { ...prev[partKey], alternatives: newAlts } };
            });
        } else {
            setError("Impossible de générer de nouvelles alternatives.");
            setPromptHistory(prev => prev.slice(0, -1));
        }
        setIsModifying(false);
    };

    const handleCustomQuery = async (partKey: PromptPartKey, query: string) => {
        if (!promptData || !apiConfig) return;
        saveToHistory();
        setIsModifying(true);
        const newAlts = await generateCustomAlternatives(apiConfig, getLabel(partKey), query, promptData[partKey].alternatives);
         if (newAlts) {
            setPromptData(prev => {
                if (!prev) return null;
                return { ...prev, [partKey]: { ...prev[partKey], alternatives: newAlts } };
            ...
            setError("Impossible de générer des alternatives personnalisées.");
            setPromptHistory(prev => prev.slice(0, -1));
        }
        setIsModifying(false);
    };

    const handleCompletePromptBlur = () => {
        if (!promptData) return;
        
        const originalText = promptOrder.map(key => promptData[key].valeur).join(', ');
        if (completePromptText === originalText) return;
        
        const newValues = completePromptText.split(', ');

        if (newValues.length !== promptOrder.length) {
            setError(`La modification a échoué car le nombre de parties ne correspond pas. (Attendu: ${promptOrder.length}, Trouvé: ${newValues.length}). Assurez-vous d'utiliser ", " comme séparateur.`);
            setCompletePromptText(originalText);
            return;
        }
        
        saveToHistory();
        const newData = { ...promptData };
        promptOrder.forEach((key, index) => {
            newData[key] = { ...newData[key], valeur: newValues[index].trim() };
        });
        setPromptData(newData);
        setError(null);
    };

    // --- Add/Delete Field Logic ---
    ...
    return (
        <div className="min-h-screen text-gray-200 flex flex-col items-center font-sans">
            <Header onStartOver={handleStartOver} hasPrompt={!!promptData} onShowSettings={() => setShowSettings(true)} onShowChatbot={handleOpenChatbot} />
            ...
            {!promptData ? (
                // Welcome View
                 <div className="w-full">
                    <header className="relative text-center mb-10 overflow-hidden">
                        ...
                        <div className="relative z-10 px-4 animate-fade-in pt-32 sm:pt-40 pb-16 sm:pb-24">
                            ...
                            <h1 className="text-5xl sm:text-6xl md:text-7xl text-white tracking-tighter font-extrabold uppercase" style={{textShadow: '0 3px 20px rgba(0,0,0,0.7)'}}>
                                    ...
                                    <div className="relative flex flex-col sm:flex-row gap-4">
                                         <div className="relative w-full flex items-center bg-black/20 border border-white/10 rounded-xl focus-within:ring-2 focus-within:ring-[#D7FE40] focus-within:shadow-[0_0_15px_rgba(215,254,64,0.4)] focus-within:border-transparent transition-all">
                                            ...
                                            <input
                                                ...
                                                className="w-full bg-transparent py-3 sm:py-4 pl-12 pr-14 text-gray-200 placeholder-gray-500 focus:outline-none"
                                                disabled={isAppBusy}
                                        ...
                                        <button
                                            ...
                                            className="bg-[#D7FE40] hover:brightness-110 disabled:bg-gray-600 disabled:text-gray-400 disabled:cursor-not-allowed disabled:shadow-none text-black font-bold py-3 sm:py-4 px-6 sm:px-8 rounded-xl transition-all duration-200 flex items-center justify-center shadow-[0_0_20px_rgba(215,254,64,0.5)] transform hover:scale-105"
                                        >
                                            {isLoading ? (
                                                <>
                ...
                // Editor View
                 <div className="w-full mx-auto animate-fade-in p-4 sm:p-8 max-w-4xl pt-24 sm:pt-28">
                    <div className="grid grid-cols-1">
                        <div>
                            <header className="text-center mb-6">
                                ...
                                <div className="inline-flex items-center justify-center bg-gray-500/10 p-1 rounded-xl mt-4">
                                    <button 
                                        onClick={() => setViewMode('structured')}
                                        disabled={isAppBusy}
                                        className={`px-4 py-1.5 text-sm font-semibold rounded-lg transition-colors ${viewMode === 'structured' ? 'bg-[#D7FE40] text-black shadow' : 'text-gray-300 hover:bg-white/5'}`}
                                    >
                                        Détaillé
                                    </button>
                                    <button 
                                        ...
                                        disabled={isAppBusy}
                                        className={`px-4 py-1.5 text-sm font-semibold rounded-lg transition-colors ${viewMode === 'complete' ? 'bg-[#D7FE40] text-black shadow' : 'text-gray-300 hover:bg-white/5'}`}
                                    >
                                        Complet
                                    </button>
                                </div>
                            ...
                            <div>
                                ...
                                ) : promptData && viewMode === 'structured' ? (
                                    <div className="bg-gray-500/10 backdrop-blur-xl border border-white/10 p-4 sm:p-8 rounded-2xl shadow-2xl">
                                        <div 
                                            ...
                                            {promptOrder.map((key, index) => (
                                                <React.Fragment key={key}>
                                                    ...
                                                    <div
                                                        ...
                                                        className={`group grid grid-cols-[auto_1fr_auto] gap-x-2 sm:gap-x-3 items-start my-3 transition-all duration-300 ${draggedIndex === index ? 'opacity-50 scale-95 shadow-2xl' : 'opacity-100 scale-100'}`}
                                                        draggable={!isAppBusy}
                                                        onDragStart={() => handleDragStart(index)}
                                                        onDragEnd={handleDragEnd}
                                                        onTouchStart={() => handleTouchStart(index)}
                                                    >
                                                        <div className="cursor-grab text-gray-400 hover:text-white p-2 pt-6 touch-none" aria-label="Réorganiser">
                                                            <i className="fa-solid fa-grip-vertical"></i>
                                                        </div>
                                                        <div className="w-full">
                                                            <label className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-1 block">
                                                                {getLabel(key)}
                                                            </label>
                                                            <button
                                                                onClick={() => handleOpenSidePanel(key)}
                                                                disabled={isAppBusy}
                                                                className="w-full text-left bg-black/20 border border-white/10 rounded-lg p-3 text-gray-200 focus:outline-none focus:ring-2 focus:ring-[#D7FE40] focus:bg-black/30 transition-all hover:bg-white/5 disabled:opacity-70 disabled:cursor-not-allowed min-h-[50px] flex items-center"
                                                            >
                                                        ...
                                                        <button
                                                            ...
                                                            disabled={isAppBusy}
                                                            className="text-gray-500 w-8 h-8 flex items-center justify-center rounded-full hover:bg-red-500/20 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all mt-7"
                                ...
                                ) : promptData && viewMode === 'complete' ? (
                                    <div className="bg-gray-500/10 backdrop-blur-xl border border-white/10 p-4 sm:p-8 rounded-2xl shadow-2xl">
                                        <textarea
                                            value={completePromptText}
                                            onChange={(e) => setCompletePromptText(e.target.value)}
                                            onBlur={handleCompletePromptBlur}
                                            disabled={isAppBusy}
                                            className="w-full h-48 bg-black/20 border border-white/10 rounded-lg p-4 text-gray-200 font-mono text-base focus:outline-none focus:ring-2 focus:ring-[#D7FE40] transition-all"
                                        />
                                    </div>
                                ) : null }
                                        ...
                                        <div>
                                            ...
                                            <div className="flex flex-col sm:flex-row gap-4">
                                                <input
                                                    ...
                                                    className="flex-grow bg-black/20 border border-white/10 rounded-xl p-3 sm:p-4 text-gray-200 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#D7FE40] focus:shadow-[0_0_15px_rgba(215,254,64,0.4)] focus:border-transparent transition-all"
                                                    disabled={isAppBusy}
                                                ...
                                                <button
                                                    ...
                                                    className="bg-white/10 hover:bg-white/20 border border-white/20 disabled:bg-gray-800 disabled:text-gray-500 disabled:border-transparent text-gray-200 font-semibold py-3 sm:py-4 px-4 sm:px-6 rounded-xl transition-all duration-200 flex items-center justify-center"
                                                >
                                        ...
                                        <div className="text-center pt-6 border-t border-white/10">
                                            <div className="flex flex-wrap gap-4 justify-center">
                                                ...
                                                <button
                                                    ...
                                                    disabled={promptHistory.length === 0 || isAppBusy}
                                                    className="bg-white/10 hover:bg-white/20 border border-white/20 disabled:bg-gray-800 disabled:text-gray-500 disabled:border-transparent text-gray-200 font-bold py-3 px-6 rounded-xl transition-all duration-300 w-full sm:w-auto flex items-center justify-center"
                                                ...
                                                <button
                                                    onClick={() => handleCopy(viewMode === 'complete' ? completePromptText : undefined)}
            ...
            {/* Side Panels & Modals */}
            {activeSidePanelKey && promptData && (
                <SidePanel
                    activeKey={activeSidePanelKey}
                    activeLabel={getLabel(activeSidePanelKey)}
                    promptData={promptData}
                    onClose={() => setActiveSidePanelKey(null)}
                    onUpdate={handleUpdateFromSidePanel}
            ...
            {showChatbot && apiConfig && (
                <Chatbot
                    ...
                    setPersona={setChatbotPersona}
                    conversations={chatbotConversations}
                    setConversations={setChatbotConversations}
                />
            )}
            
            {showHistory && promptData && (
                <HistoryPanel
